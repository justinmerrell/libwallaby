...
unsigned int Wallaby::readRegister32b(unsigned char address, const unsigned char * alt_read_buffer)
{
	...
	unsigned int value =
			static_cast <unsigned int> 
			  (static_cast <unsigned char>(readRegister(address)) << 24
			| static_cast <unsigned char>(readRegister(address+1)) << 16
			| static_cast <unsigned char>(readRegister(address+2)) << 8
			| static_cast <unsigned char>(readRegister(address+3)));

	std::cout << address+3 << std::endl;
	std::cout << readRegister(address) << std::endl;
	std::cout << readRegister(address+1) << std::endl;
	std::cout << readRegister(address+2) << std::endl;
	std::cout << readRegister(address+3) << std::endl;
